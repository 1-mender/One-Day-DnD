...
export default function DMBestiaryEditorModal({ controller }) {
  ...
  return (
    <Modal open={open} title={edit ? "Редактировать" : "Новый монстр"} onClose={() => setOpen(false)}>
      <div className="list dm-bestiary-editor">
        {err ? <div className="badge off">Ошибка: {err}</div> : null}
        <section className="dm-bestiary-editor-section">
          <div className="tf-overline">Core profile</div>
          <div className="u-fw-800">Основное</div>
          <div className="small">Имя, тип, среда и видимость монстра.</div>
          <div className="list u-mt-10">
            ...
            <label className="small">
              <input type="checkbox" checked={!!form.is_hidden} onChange={(event) => setForm({ ...form, is_hidden: event.target.checked })} disabled={readOnly} /> Скрыть для игроков
            </label>
          </div>
        </section>

        <section className="dm-bestiary-editor-section">
          <div className="tf-overline">Lore sheet</div>
          <div className="u-fw-800">Описание и способности</div>
          <div className="small">То, что увидит игрок и DM в карточке монстра.</div>
          <div className="list u-mt-10">
        ...
        <section className="dm-bestiary-editor-section">
          <div className="tf-overline">Media</div>
          <div className="u-fw-800">Изображения</div>
          <div className="small">PNG/JPG/WEBP/GIF/HEIC до 5MB.</div>
          {!edit ? <div className="small u-mt-6">Сначала сохраните монстра, чтобы загрузить картинки.</div> : null}
          ...
          <div className="u-grid-auto-170 u-mt-10">
            {images.map((image) => (
