  ...
  return (
    <Modal open={open} title={edit ? "Редактировать" : "Новый монстр"} onClose={() => setOpen(false)}>
      <div className="list">
        ...
        <input value={form.name || ""} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Имя*" aria-label="Имя монстра" style={INPUT_STYLE} />
        <div className="row">
          <input value={form.type || ""} onChange={(event) => setForm({ ...form, type: event.target.value })} placeholder="Тип" aria-label="Тип монстра" style={INPUT_STYLE} />
          <input value={form.habitat || ""} onChange={(event) => setForm({ ...form, habitat: event.target.value })} placeholder="Среда" aria-label="Среда обитания" style={INPUT_STYLE} />
        </div>
        <input value={form.cr || ""} onChange={(event) => setForm({ ...form, cr: event.target.value })} placeholder="CR (число/строка)" aria-label="CR монстра" style={INPUT_STYLE} />
        <textarea value={form.description || ""} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Описание (markdown)" aria-label="Описание монстра" rows={5} style={INPUT_STYLE} />
        <textarea
        ...
        <label className="small">
          <input type="checkbox" checked={!!form.is_hidden} onChange={(event) => setForm({ ...form, is_hidden: event.target.checked })} /> Скрыть для игроков (опц.)
        </label>
