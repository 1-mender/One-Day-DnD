...
liveActivityRouter.get("/me", (req, res) => {
  ...
  const activity = getActivePlayerLiveActivity(me.player.id, { db: getDb(), kind: LIVE_ACTIVITY_KINDS.ARCADE });
...
liveActivityRouter.post("/dm/player/:id/open", dmAuthMiddleware, (req, res) => {
  ...
  if (kind !== LIVE_ACTIVITY_KINDS.ARCADE) return res.status(400).json({ error: "invalid_live_activity_kind" });
...
liveActivityRouter.post("/dm/player/:id/close", dmAuthMiddleware, (req, res) => {
  ...
  req.app.locals.io?.to(`player:${player.id}`).emit("player:minigame:closed", { activity, kind });
  emitSinglePartyEvent(req.app.locals.io, "players:updated", undefined, { partyId: player.party_id });
  return res.json({ ok: true, activity });
});
