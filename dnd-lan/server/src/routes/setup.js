...
import { dmSetupBodySchema, parseSetupRouteInput } from "./setupRouteSchemas.js";
import { createRouteInputReader } from "./routeValidation.js";

export const setupRouter = express.Router();
const readValidBody = createRouteInputReader(parseSetupRouteInput);
...
setupRouter.post("/setup", (req, res) => {
  const body = readValidBody(res, dmSetupBodySchema, req.body);
  if (!body) return;

  if (dbHasDm()) return res.status(409).json({ error: "already_setup" });

  const expectedSecret = String(process.env.DM_SETUP_SECRET || "").trim();
  const providedSecret = String(req.header("x-setup-secret") || body.setupSecret || "").trim();
  if (expectedSecret) {
